...
import com.ics.tcg.web.workflow.client.ChoiceTask;
import com.ics.tcg.web.workflow.client.FixedFor;
import com.ics.tcg.web.workflow.client.GwtDiagramsExample;
import com.ics.tcg.web.workflow.client.LoopTask;
import com.ics.tcg.web.workflow.client.MyPanel;
import com.ics.tcg.web.workflow.client.NonFixedFor;
import com.ics.tcg.web.workflow.client.ServerStatus_Service;
import com.ics.tcg.web.workflow.client.ServerStatus_ServiceAsync;
import com.ics.tcg.web.workflow.client.SimpleTask;
import com.ics.tcg.web.workflow.client.TimerNode;
import com.ics.tcg.web.workflow.client.Workflowtasknode;
...
import com.ics.tcg.web.workflow.client.common.myline.CustomConnection;
import com.ics.tcg.web.workflow.client.common.myline.CustomUIObjectConnector;
import com.ics.tcg.web.workflow.client.common.myline.StraightLineConnection;
...
//	AbsolutePanel panel;
	...
	public GwtDiagramsExample gde;
	public Workflowtasknode lastNode; // 指向工作流中最后一个节点的引用

	public List<SimpleTask> list = new ArrayList<SimpleTask>(); // 没用，待处理
	...
	public List<SimpleTask> getList() {
		return list;
	}

	public void setList(List<SimpleTask> list) {
		this.list = list;
	}

	public Panel_Overview overview;

	public DiagramBuilderExample(GwtDiagramsExample gwtdiagramsexample,
	...
	protected void createContents() {
		...
		DOM.setStyleAttribute(leftAbsolutePanel.getElement(), "borderRight", "2px solid #DDDDDD");
	...
	public void create_type() {
		...
		label.sinkEvents(Event.ONMOUSEOVER | Event.ONMOUSEOUT |Event.ONLOSECAPTURE);
		...
		label2.sinkEvents(Event.ONMOUSEOVER | Event.ONMOUSEOUT |Event.ONLOSECAPTURE);
		...
		label3.sinkEvents(Event.ONMOUSEOVER | Event.ONMOUSEOUT|Event.ONLOSECAPTURE);
		...
		label4.sinkEvents(Event.ONMOUSEOVER | Event.ONMOUSEOUT | Event.ONLOSECAPTURE);
...
//		overview.service_Service.getUserAbservices(overview.userid,
...
//
//					@Override
//					public void onSuccess(List<User_Service_Client> services) {
//						for (int i = 0; i < services.size(); i++) {
//							serviceNameList.add(services.get(i).abservicename);
//							System.out.println(services.get(i).abservicename);
//
//						}
//						service_info_list = serviceNameList;
//						// hashMap = new HashMap<String, Integer>(); //
//						// 保证创建的service和控制组件编号从小到大
//
//						// 根据服务的个数，动态设置服务显示列表的大小
//						int service_count = service_info_list.size();
//						leftverticalPanel.setSize("100", Integer
//								.toString(30 * service_count));
//
...
//								}
//							};
//							label.sinkEvents(Event.ONMOUSEOVER
//									| Event.ONMOUSEOUT);
//							label.addStyleName("service-label");
//							toolboxDragController.makeDraggable(label);
//
...
//	}
	...
	public void create_service(){
		...
		for (int i = 0; i < overview.user_Service_Clients.size(); i++) {
			serviceNameList.add(overview.user_Service_Clients.get(i).abservicename);
		}
		service_info_list = serviceNameList;
		// hashMap = new HashMap<String, Integer>(); //
		// 保证创建的service和控制组件编号从小到大
		...
		// 根据服务的个数，动态设置服务显示列表的大小
		int service_count = service_info_list.size();
		leftverticalPanel.setSize("180", Integer
				.toString(30 * service_count));
		...
		for (Iterator iterator = service_info_list.iterator(); iterator
			...
			label.sinkEvents(Event.ONMOUSEOVER
					| Event.ONMOUSEOUT | Event.ONLOSECAPTURE);
			label.addStyleName("service-label");
			toolboxDragController.makeDraggable(label);
	...
	protected void createConnector(Widget widget, AbsolutePanel panel) {
		...
		if (proxy.getText().equals(SIMPLENODE)) {
			l = new SimpleTask(proxy.getText(), this); // 将diagrambuilder的引用传给simpletask
			...
			l = new SimpleTask(service_name, this);
	...
	private class ToolboxDragController extends PickupDragController {
		...
		protected Widget maybeNewDraggableProxy(Widget draggable) {
			...
			if (node_name.equals(SIMPLENODE)) {
				proxy = new SimpleTask(node.getText());
	...
	public void remove_task() {
		for (Iterator i = selected_taskList.listIterator(); i.hasNext();) {
			...
			SimpleTask parentSimpleTask = (SimpleTask) w.getParent()
					.getParent();
	...
	public Client_Workflow save_workflow_in_workflowmode() {
				...
				if (workflowtasknode.getText().equals(LOOPNODE)) {
					...
					if (loop.isIsstart()) {
						...
						if (starter_count > 1) {
							...
							check = Window.confirm("Force to end and return to null, sure?");
							return null;
						}
					}
					...
					if (choiceTask.outport_count == 0) {
						...
						check = Window.confirm("ChoiceTask should have an output, force to end and return to null, sure?");
						return null;
					}
					// 在工作流类中，设置起始节点
					if (choice.isIsstart()) {
					...
					SimpleTask simpleTask = (SimpleTask) workflowtasknode;
					...
					if (service.isIsstart()) {
						...
						if (starter_count > 1) {
							...
							check = Window.confirm("Begin with multi nodes, force to end and return to null, sure?");
							return null;
						}
					}
		...
		for (int index = 0; index < node_count; index++) {
			...
			if (widget instanceof Workflowtasknode) { // 确保widget是workflow的节点
				...
				if (workflowtasknode.getText().equals(LOOPNODE)) { // 节点是loopnode时
					...
					ArrayList<SimpleTask> backTaskList = new ArrayList<SimpleTask>();
					...
					for (Iterator iterator2 = collection.iterator(); iterator2
						...
						SimpleTask back_end_simpletask = (SimpleTask) back_end_panelPanel
								.getParent().getParent();
					...
					ArrayList<SimpleTask> preTaskList = new ArrayList<SimpleTask>();
					...
					for (Iterator iterator2 = collection2.iterator(); iterator2
						...
						SimpleTask pre_end_simpletask = (SimpleTask) pre_end_panelPanel
								.getParent().getParent();
		...
		if (starter_count == 0 || ender_count == 0) {
			...
			check = Window.confirm("No begin-node or end-node, force to end and return to null, sure?");
			return null;
		}
		if (allNodesHasBeenSave == false) {
			System.out.println("还有节点未完成服务配置，强行结束保存过程");
			check = Window.confirm("Some nodes are not configured, force to end and return to null, sure?");
			return null;
		}
		...
		if (client_InputPort.getBeforeNodes() != null
			...
			check = Window.confirm("There's node before begin-node, force to end and return to null, sure?");
			return null;
		}
		...
		for (Iterator<Client_Node> iterator = workFlow.getLastNodes()
			...
			if (lastNode.getName().equals(FIXEDFORNODE)) {
				...
				if (client_OutputPort.getHasNextNodes() != null
					...
					System.out.println("弹出一个对话框，终止节点节点后还有节点，强行结束保存过程");
					check = Window.confirm("There's node after end-node, force to end and return to null, sure?");
					return null;
				}
			} else if (lastNode.getName().equals(NONFIXEDNUMFOR)) {
		...
		if (client_Workflow != null) {
			...
			for (int index = 0; index < node_count; index++) {
				...
				if (client_Node.getName().equals(LOOPNODE)) {
					...
					LoopTask loop = new LoopTask(client_Node.getName(), this,overview);
					...
					panel.add(loop, client_Loop.getX()
							, client_Loop.getY());
					...
					panel.add(choice, client_Choice.getX(), client_Choice.getY());
					...
					SimpleTask service = new SimpleTask(client_Node.getName(),
							this);
			...
			for (int index = 0; index < node_count; index++) {
				...
				if (client_Node.getName().equals(LOOPNODE)) { // 节点是loopnode时
					...
					Client_ServiceTask client_ServiceTask = (Client_ServiceTask) client_Node;
					SimpleTask service = (SimpleTask) workflowMap
