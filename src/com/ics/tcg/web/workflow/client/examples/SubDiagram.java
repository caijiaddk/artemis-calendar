...
public class SubDiagram extends DiagramBuilderExample {
	...
	public SubDiagram(LoopTask l, final GwtDiagramsExample gwtDiagramsExample,
			final Panel_Overview overview) {
		super(gwtDiagramsExample, overview);
		
		/** set panel */
		getArea().setSize(
				Integer.toString(overview.workflowpanel.workflowPanel_inner.getOffsetWidth() - 30),
				Integer.toString(overview.workflowpanel.workflowPanel_inner.getOffsetHeight() - 70));
		getPanel().setSize("1200","800");
		scrollPanel.setSize(Integer
				.toString(overview.workflowpanel.workflowPanel_inner.getOffsetWidth() - 210), Integer
				.toString(overview.workflowpanel.workflowPanel_inner.getOffsetHeight() - 70));
				...
				getArea()
						.setSize(
								...
								Integer.toString(overview.workflowpanel.workflowPanel_inner
										.getOffsetHeight() - 70));
				scrollPanel
						.setSize(Integer.toString(overview.workflowpanel.workflowPanel_inner
								.getOffsetWidth() - 210),
								Integer.toString(overview.workflowpanel.workflowPanel_inner
										.getOffsetHeight() - 70));
	...
	public Client_Workflow save_workflow_in_workflowmode() {
				...
				if (workflowtasknode.getText().equals(LOOPNODE)) {
					...
					if (loop.isIsstart()) {
						...
						if (starter_count > 1) {
							...
							check = Window.confirm("Just one end-node,force to end and return to null, sure?");
							return null;
						}
					}
					if (loop.isIsfinished()) {
						...
						if (ender_count > 1) {
							...
							check = Window.confirm("Just one endnode, force to end and return to null, sure?");
							return null;
						}
					}
					...
					if (choice.isIsstart()) {
						...
						if (starter_count > 1) {
							System.out.println("弹出一个对话框，起始节点多于一个，强行结束保存过程");
							check = Window.confirm("Just one begin-node, force to end and return to null, sure?");
							return null;
						}
					}
		...
		if (allNodesHasBeenSave == false) {
			...
			check = Window.confirm("At learst one node is not configured, force to end and return to null, sure?");
			return null;
		}
		...
		if (lastNode != null) {
			...
			if (valid == false) {
				...
				check = Window.confirm("end-node contains no loop value, force to end and return to null, sure?");
				return null;
			}
		} else {
			System.out.println("弹出一个对话框，终止节点未设置，强行结束保存过程");
			check = Window.confirm("no end-node, force to end and return to null, sure?");
			return null;
		}
		...
		if (client_InputPort.getBeforeNodes() != null
			...
			check = Window.confirm("Node before begin-node, force to end and return to null, sure?");
			return null;
		}
		...
		if (lastNode.getName().equals(FIXEDFORNODE)) {
			...
			if (client_OutputPort.getHasNextNodes() != null
				...
				check = Window.confirm("Node after end-node, force to end and return to null, sure?");
				return null;
			}
		...
		if (client_Workflow != null) {
			...
			for (int index = 0; index < node_count; index++) {
				...
				if (client_Node.getName().equals(LOOPNODE)) {
					...
					panel.add(choice, client_Choice.getX()
							, client_Choice.getY()
							);
			...
			if (paramInfo.isPrimitive()) {
				...
				for (Iterator<ParamInfo> iterator = paramInfo
					...
					if (check_ParaminfoWeatherHasVariableInput(
							(ParamInfo) iterator.next(), loopTask) == true) {
						return true;
					}
				}
				return false;
