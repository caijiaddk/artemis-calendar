...
import com.ics.tcg.web.workflow.client.common.myline.CustomUIObjectConnector;
import com.ics.tcg.web.workflow.client.examples.Client_IfCondition;
...
public class ChoiceTask extends SimpleTask {
	...
	public List<Client_IfCondition> IfConditionlist = new ArrayList<Client_IfCondition>(10);
//	List<Label> 
	...
	public void setTextLabel(Label label,String labelText) {
		...
		if (labelText.equals("DEL")) {
			...
			Collection<Connection> cons = CustomUIObjectConnector.getWrapper(myPanel).getConnections();
			...
			//æ›´æ–°è¿žçº¿çš„æ˜¾ç¤º
			for(Iterator iterator=getconnectorList().iterator();iterator.hasNext();){
				CustomUIObjectConnector c = CustomUIObjectConnector.getWrapper((MyPanel)iterator.next());
				if(c!=null){
					c.update();
				}
			}
...
//		});
		...
		Client_IfCondition client_IfCondition = new Client_IfCondition(ori_TextLabel.getText());
...
////			}
//			if(!simpleTask.getText().equals("Choice")){
//				//check whether preNode has been configured ,then determine how to display the preNode information
//				if(simpleTask.getServiceConfigure().getMethodInfo()!=null){
//					TreeItem serviceName = staticTree.addItem(simpleTask.getTitle());
//				    	MethodInfo methodInfo = simpleTask.getServiceConfigure().getMethodInfo();
//				    	if(methodInfo!=null){
//							TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
//							addParamInfoInTree(output, methodInfo.getOutputInfo().getParamInfoArray());
//				    	}
...
//		}
		...
		for(Iterator<SimpleTask> iterator = preTaskList.iterator();iterator.hasNext();){
			...
			if(!simpleTask.getText().equals("Choice")){
				if(simpleTask.getText().equals("Loop")){
					LoopTask loopTask = (LoopTask)simpleTask;
					if(loopTask.getServiceConfigure().getMethodInfo()!=null){
						TreeItem serviceName = staticTree.addItem(loopTask.getTitle());
				    	MethodInfo methodInfo = loopTask.getServiceConfigure().getMethodInfo();
				    	if(methodInfo!=null){
							TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
							addParamInfoInTree(output, methodInfo.getOutputInfo().getParamInfoArray());
				    	}
				...
				else if(simpleTask.getText().equals("TLoop")){
					NonFixedFor nonFixedFor = (NonFixedFor)simpleTask;
					if(nonFixedFor.getServiceConfigure().getMethodInfo()!=null){
						TreeItem serviceName = staticTree.addItem(nonFixedFor.getTitle());
				    	MethodInfo methodInfo = nonFixedFor.getServiceConfigure().getMethodInfo();
				    	if(methodInfo!=null){
							TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
							addParamInfoInTree(output,methodInfo.getOutputInfo().getParamInfoArray());
				    	}
	...
	public void addoutput() {
		...
		for(Iterator<Label> iterator = label_array.iterator();iterator.hasNext();){
...
//			int addheight = 20;
			...
			Client_IfCondition client_IfCondition = new Client_IfCondition(newlabel.getText());
			...
			//update connections
			for(Iterator<MyPanel> iterator=getconnectorList().iterator();iterator.hasNext();){
				CustomUIObjectConnector c = CustomUIObjectConnector.getWrapper((MyPanel)iterator.next());
				if(c!=null){
					c.update();
				}
...
//				        			this.hide();
        ...
        popupPanel.addCloseHandler(new CloseHandler<PopupPanel>(){
			...
			public void onClose(CloseEvent<PopupPanel> event) {
				// TODO Auto-generated method stub
				AbsolutePanel absolutePanel = (AbsolutePanel)popupPanel.getWidget();
		        ...
		        if(paramInfo_for_configure.getParamTypeName().equals("Integer")){
		        	new_condition = get_condition_value(absolutePanel, condition);
		        }
		        else{
		        	new_condition = get_another_condition_value(absolutePanel, condition);
		        }
				
				setTextLabel(label, new_condition);					//ç»™ä»£è¡¨è¾“å‡ºæ�¡ä»¶çš„labelèµ‹å€¼
				if(!new_condition.equals("DEL")){
					int index = label_array.indexOf(label);
					Client_IfCondition client_IfCondition = IfConditionlist.get(index);
	...
	public void determine_popup_position(PopupPanel popupPanel,Widget backgroud_region,int x,int y){
		
		int back_panel_center_x = backgroud_region.getAbsoluteLeft()+backgroud_region.getOffsetWidth()/2;
        int back_panel_center_y = backgroud_region.getAbsoluteTop()+backgroud_region.getOffsetHeight()/2;
	...
	public AbsolutePanel generate_another_panel_for_popup(){
		...
		listBox.addItem(" ");
		System.out.println("paramInfo_for_configure"+paramInfo_for_configure.getParamName());
...
//			listBox.addItem("Rainy");
	...
	public String get_condition_value(AbsolutePanel absolutePanel,String condition){
		...
		if(image.isVisible()){
			...
			if(isInt(textString1)&&isInt(textString)){
				expression = textString+listBoxString+label.getText()+listBoxString1+textBox1.getText();
			}
	...
	public String get_another_condition_value(AbsolutePanel absolutePanel,String condition){
		String expression = condition;
	 ...
	 public boolean check_ParamInfoWeatherComefrom(SimpleTask simpleTask,String contentPath){
	 ...
	 *            Handler object for performing actions once label is updated.
	 */
	public ChoiceTask(String labelText, DiagramBuilderExample d) {
		dbe = d;
