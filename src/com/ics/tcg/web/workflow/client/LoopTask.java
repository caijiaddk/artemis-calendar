...
import com.ics.tcg.web.workflow.client.examples.Client_NonFixedNumFor;
...
public class LoopTask extends SimpleTask{
...
//	List outport_info;
	...
	private void createEditableLabel(String labelText, String in_name,
		...
		SetStartCommand setStartCommand = new SetStartCommand(this,dbe,setstart);
		setstart.setCommand(setStartCommand);
		
		setfinish = new MenuItem("isfinish",(Command)null);
		SetFinishCommand setFinishCommand = new SetFinishCommand(this,dbe,setfinish);
    ...
    public LoopTask(String labelText, String in, String out, String fault,
			DiagramBuilderExample d,Panel_Overview overview) {
	 ...
	 */
	public LoopTask(String labelText, DiagramBuilderExample d,Panel_Overview overview) {
		this.overview = overview;
		dbe = d;
	...
	public void setServiceConfigure(){
		...
		if(sub_workflow!=null){
			if(sub_workflow.getHasLastNode().getName().equals("Loop")){
				Client_Loop client_Loop = (Client_Loop)sub_workflow.getHasLastNode();
				methodInfo = client_Loop.getServiceConfigure().getMethodInfo();
			}
			else if(sub_workflow.getHasLastNode().getName().equals("TLoop")){
				Client_NonFixedNumFor client_NonFixedNumFor = (Client_NonFixedNumFor)sub_workflow.getHasLastNode();
				methodInfo = client_NonFixedNumFor.getServiceConfigure().getMethodInfo();
			}
			else if(sub_workflow.getHasLastNode().getName().equals("Choice")){
				
			}
			else{
				Client_ServiceTask client_ServiceTask = (Client_ServiceTask)sub_workflow.getHasLastNode();
				methodInfo = client_ServiceTask.getService().getHasConfiguration().getServiceConfigure().getMethodInfo();
			}
		}
...
//		label.setSize("19px", "9px");
		...
		addImage.addClickHandler(new ClickHandler(){
			...
			public void onClick(ClickEvent event) {
				...
				outer_absolutePanel.setHeight(Integer.toString(150*conditionNumber));
				outer_absolutePanel.add(newInnerAbsolutePanel, 21, 150*(conditionNumber-1));
			}
		...
		delImage.addClickHandler(new ClickHandler(){
			...
			public void onClick(ClickEvent event) {
				...
				if(conditionNumber>1){
					AbsolutePanel lastInnerAbsolutePanel = (AbsolutePanel)outer_absolutePanel.getWidget(1+conditionNumber);
					lastInnerAbsolutePanel.removeFromParent();
					conditionNumber--;
					outer_absolutePanel.setHeight(Integer.toString(150*conditionNumber));
					
					refreshWhileCondition(whileCondition_textBox,outer_absolutePanel);
				}
			}
			...
			public void onClick(ClickEvent event) {
				...
				if(textBox_error==true){
					...
					if(whileCondition_textBox.getValue().equals("")){
						...
						whilecondition = whileCondition_textBox.getValue();		//save the whilecondition
	...
	public AbsolutePanel generateAbsolutePanelForConditionSet(final TextBox conditionTextBox){
...
//		inner_absolutePanel.add(compareLabel_textBox, 117, 61);
		...
		compareLabel_listBox.addChangeListener(new ChangeListener(){
			...
			public void onChange(Widget sender) {
				// TODO Auto-generated method stub
				AbsolutePanel innerAbsolutePanel = (AbsolutePanel)sender.getParent();
				AbsolutePanel outterAbsolutePanel = (AbsolutePanel)innerAbsolutePanel.getParent();
		...
		rightOperator_textBox.addFocusListener(new FocusListener(){
			...
			public void onLostFocus(Widget sender) {
				...
				if(isInt(s)){
					...
					AbsolutePanel innerAbsolutePanel = (AbsolutePanel)parentDeckPanel.getParent();
					AbsolutePanel outterAbsolutePanel = (AbsolutePanel)innerAbsolutePanel.getParent();
//					conditionTextBox.setValue(leftOperator_textBox.getValue()+compareLabel_listBox.getValue(compareLabel_listBox.getSelectedIndex())+s);
					...
					for(int i = 1;i<=conditionNumber;i++){
						AbsolutePanel newinnerAbsolutePanel = (AbsolutePanel)outterAbsolutePanel.getWidget(i+1);
						...
						if(leftOperatorValue!=null&&!leftOperatorValue.equals("")){
							ListBox compareBox = (ListBox)newinnerAbsolutePanel.getWidget(3);
							String compareBoxValue = compareBox.getValue(compareBox.getSelectedIndex());
							
							DeckPanel deckPanel = (DeckPanel)newinnerAbsolutePanel.getWidget(5);
							...
							if(index==0){
								...
								rightOperatorValue = listBox.getValue(listBox.getSelectedIndex());
							}
							
							String conditionString = leftOperatorValue+compareBoxValue+rightOperatorValue;
							if(i==1){
								conditionTextBox.setValue(conditionString);
							}
							else{
								conditionTextBox.setValue(conditionTextBox.getValue()+"&&"+conditionString);
							}
						}
...
//					int current_level = dbe.gde.tabs.getWidgetCount();
					...
					String new_contentPath =  contentPath.replace(serviceName, serviceID);
	    ...
	    if(client_Node.getName().equals("Choice")){
			...
			if(client_Loop.getServiceConfigure().getMethodInfo()!=null){
				TreeItem serviceName = staticTree.addItem(client_Loop.getName());
		    	MethodInfo methodInfo = client_Loop.getServiceConfigure().getMethodInfo();
		    	if(methodInfo!=null){
					TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
					addParamInfoInTree(output, methodInfo.getOutputInfo().getParamInfoArray());
		    	}
			}
	    }
			...
			if(client_NonFixedNumFor.getServiceConfigure().getMethodInfo()!=null){
				TreeItem serviceName = staticTree.addItem(client_NonFixedNumFor.getName());
		    	MethodInfo methodInfo = client_NonFixedNumFor.getServiceConfigure().getMethodInfo();
		    	if(methodInfo!=null){
					TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
					addParamInfoInTree(output, methodInfo.getOutputInfo().getParamInfoArray());
		    	}
	    	...
	    	TreeItem serviceName = staticTree.addItem(client_ServiceTask.getName());
	    	MethodInfo methodInfo = client_ServiceTask.getService().getHasConfiguration().getServiceConfigure().getMethodInfo();
			TreeItem output = serviceName.addItem(methodInfo.getMethodName()+".OutputInfo");
			addParamInfoInTree(output, methodInfo.getOutputInfo().getParamInfoArray());
	    }
	    
//		for(Iterator<SimpleTask> iterator = preTaskList.iterator();iterator.hasNext();){
...
//				for(Iterator<MethodInfo> iterator2 = simpleTask.getServiceInfo().getMethodInfoArray().iterator();iterator2.hasNext();){
...
//					setHasFinishedConfigure(true);
					TextBox leftOperator = (TextBox)innerAbsolutePanel.getWidget(1);
					leftOperator.setValue(textBox.getValue());
					
					Label compareLabel = (Label)innerAbsolutePanel.getWidget(2);
					compareLabel.removeStyleName("gray-text");
					
					ListBox compareBox = (ListBox)innerAbsolutePanel.getWidget(3);
					...
					Label rightOperator = (Label)innerAbsolutePanel.getWidget(4);
					rightOperator.removeStyleName("gray-text");
					
					DeckPanel rightBox = (DeckPanel)innerAbsolutePanel.getWidget(5);
					if(paramInfo_for_configure.getParamTypeName().equals("Integer")){
						...
						if(paramInfo_for_configure.getParamTypeName().equals("String")){
							if(DataLib.whetherInDataLib(paramInfo_for_configure.getParamName())){
								String[] result = paramInfo_for_configure.getParamEnums().split(",");
...
//					refreshWhileCondition(conditionTextBox, outterAbsolutePanel);
	...
	public void refreshWhileCondition(final TextBox conditionTextBox,final AbsolutePanel absolutePanel){
	...
	public boolean checkConditionConfigure(AbsolutePanel outterAbsolutePanel){
		boolean valid=true;
		for(int i=1;i<=conditionNumber;i++){
			AbsolutePanel innerAbsolutePanel = (AbsolutePanel)outterAbsolutePanel.getWidget(i+1);
	...
	public void addParamInfoInTree(TreeItem treeItem,ParamInfo paramInfo){
		  if(paramInfo.isPrimitive()){
			  treeItem.addItem(paramInfo.getParamName()+"	"+"("+paramInfo.getParamTypeName()+")");
			  name_for_paraminfo.put(paramInfo.getParamName(), paramInfo);				//将paraminfo的名字与paraminfo的引用绑定起来
		  }
		  else{
			  TreeItem currentTreeItem = treeItem.addItem(paramInfo.getParamTypeName());
			  for(Iterator<ParamInfo> iterator = paramInfo.getFieldInfoArray().iterator();iterator.hasNext();){
